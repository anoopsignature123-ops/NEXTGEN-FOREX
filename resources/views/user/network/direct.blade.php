    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-4 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-2xl sm:text-3xl font-black text-gold-gradient font-heading uppercase">DIRECT MEMBERS
                    DIRECTORY</h1>
                <p class="text-xs text-neutral-300 mt-1">Manage and track your direct referrals, binary placements, and team
                    status.</p>
            </div>

            <div class="flex items-center gap-3 w-full lg:w-auto">
                <a href="{{ route('user.network.tree') }}"
                    class="w-full lg:w-auto px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center justify-center gap-2 shrink-0">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="p-3.5 sm:p-5 rounded-2xl bg-panel border border-amber-500/30 shadow-lg min-w-0">
                <div class="flex items-center justify-between gap-1">
                    <span class="text-[10px] sm:text-xs font-bold uppercase text-neutral-400 truncate">Total Direct
                        Referrals</span>
        ...
        <div class="pdf-package-card p-4 sm:p-6 shadow-2xl rounded-2xl space-y-6">
            <!-- Search & Filter Form -->
            <form action="{{ route('user.network.direct') }}" method="GET"
                class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 border-b border-amber-500/20 pb-4">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full sm:w-auto">
                    <select name="position" onchange="this.form.submit()"
                        class="w-full sm:w-auto px-3.5 py-2.5 rounded-xl bg-bg border border-amber-500/40 text-amber-400 font-bold text-xs focus:outline-none cursor-pointer">
                        ...
                        @forelse($directs as $direct)
                            <tr class="hover:bg-amber-500/10 transition">
                                ...
                                <td class="p-4 text-center">
                                    <a href="{{ route('user.network.tree', ['code' => $direct->referral_code]) }}"
                                        class="px-3 py-1.5 rounded-lg bg-amber-500/20 text-amber-300 hover:bg-amber-500/40 transition text-xs font-bold inline-flex items-center gap-1">
