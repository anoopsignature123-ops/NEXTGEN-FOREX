    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-neutral-300 mt-1">Interactive binary placement tree hierarchy for your team downline.
                </p>
                </div>
            ...
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <form action="{{ route('user.network.tree') }}" method="GET" class="flex items-center gap-2 w-full lg:w-auto">
                    <div class="relative flex-1 lg:w-64">
                        ...
                        <input type="text" name="code" value="{{ request('code') }}" placeholder="Search referral code..."
                    ...
                    <button type="submit"
                        class="px-4 py-2.5 rounded-xl bg-amber-500 text-black font-black text-xs uppercase tracking-wider hover:bg-amber-400 transition shrink-0">
                        Find Tree Node
                    </button>
                    </form>
                    </div>
        ...
        <div class="bg-panel p-4 sm:p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6 overflow-x-auto">
            <div
                class="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-amber-500/20 pb-4 gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs sm:text-sm font-black text-amber-400 font-mono">{{ $rootUser->name }}
                        ({{ $rootUser->referral_code }})</span>
                    </div>

                <a href="{{ route('user.network.tree') }}"
                    class="px-3 py-1.5 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/40 hover:bg-amber-500/40 transition text-xs font-bold flex items-center gap-1 shrink-0">
