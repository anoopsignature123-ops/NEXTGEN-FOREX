...
<div class="w-full space-y-6">
    ...
    <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-neutral-300 mt-1">Deposit USDT (BEP20) into your Deposit Wallet to purchase NextGen Forex Investment Packages.</p>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ route('user.deposits.history') }}" class="px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center gap-2 shrink-0">
    ...
    <div class="grid grid-cols-1 grid-2-cards gap-6">
        ...
        <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-5 flex flex-col justify-between">
            <div class="space-y-4">
                ...
                <div class="flex flex-col items-center justify-center p-5 bg-bg rounded-2xl border border-amber-500/30 text-center space-y-4">
                    <div class="p-2.5 bg-white rounded-2xl shadow-xl border-4 border-amber-400">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data={{ $usdtWalletAddress }}" alt="USDT BEP20 Deposit QR" class="w-40 h-40 rounded-lg">
                    </div>

                    <div class="w-full space-y-1">
                        <span class="text-[11px] font-extrabold text-amber-400 uppercase tracking-wider block">OFFICIAL USDT (BEP20) ADDRESS</span>
                        <div class="p-3 rounded-xl bg-black border border-amber-500/40 text-xs font-mono text-amber-300 break-all flex items-center justify-between gap-2 shadow-inner">
                            ...
                            <button onclick="navigator.clipboard.writeText('{{ $usdtWalletAddress }}'); showToast('Copied!', 'Wallet address copied.', 'success');" class="px-3 py-1.5 rounded-lg bg-amber-500 text-black font-black text-xs uppercase hover:bg-amber-400 transition shrink-0 shadow">
                                Copy
                            </button>
                        </div>
                    </div>
            ...
            <div class="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-neutral-300 space-y-1 leading-relaxed mt-2">
                ...
                <form action="{{ route('user.deposits.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4" id="depositSubmitForm">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" min="10" name="amount" value="{{ old('amount', 100) }}" placeholder="100.00" class="w-full pl-8 pr-4 py-3 rounded-xl bg-bg border border-amber-500/40 text-white font-mono font-bold text-xs focus:outline-none focus:border-amber-400" required>
                        </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="txn_hash" value="{{ old('txn_hash') }}" placeholder="e.g. 0x3a4b...8f9c" class="w-full px-4 py-3 rounded-xl bg-bg border border-amber-500/40 text-white font-mono text-xs focus:outline-none focus:border-amber-400" required>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Payment Proof Screenshot (Optional)</label>
                        <input type="file" name="proof_image" accept="image/*" class="w-full px-4 py-3 rounded-xl bg-bg border border-amber-500/40 text-white text-xs focus:outline-none focus:border-amber-400">
                    </div>
                </form>
            </div>

            <div class="pt-4 border-t border-amber-500/20">
                <button type="submit" form="depositSubmitForm" class="w-full py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg hover:scale-[1.01] transition flex items-center justify-center gap-2">
                    <i data-lucide="send" class="w-4 h-4 text-black"></i> Submit Deposit Request
                </button>
            </div>
        </div>
