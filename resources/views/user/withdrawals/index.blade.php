    ...
    <div class="w-full space-y-6 font-sans">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-xs text-neutral-400 mt-1">Withdraw your earnings 24x7 to your USDT (BEP20) crypto wallet with
                    instant 10% deduction calculation</p>
            </div>

            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                <a href="{{ route('user.withdrawals.history') }}"
                    class="px-4 py-2.5 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs hover:bg-amber-500/30 transition flex items-center justify-center gap-2">
        ...
        <div class="flex flex-col lg:flex-row gap-6 w-full items-start">
            ...
            <div class="w-full lg:w-1/2 bg-panel p-5 sm:p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-5">
                <div class="flex items-center justify-between border-b border-amber-500/20 pb-3">
                    ...
                    <span
                        class="px-2.5 py-1 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[10px] font-black font-mono">
                        24x7 Active
                    </span>
                </div>
                ...
                <form action="{{ route('user.withdrawals.store') }}" method="POST" class="space-y-4"
                    onsubmit="return confirm('Are you sure you want to submit this withdrawal request?');">
                    @csrf
                    ...
                    <div class="p-3.5 rounded-xl bg-black/60 border border-amber-500/40 flex items-center justify-between">
                        <div>
                            <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider block">EARNING
                                WALLET</span>
                            <span
                                class="text-xl font-black font-mono text-emerald-400">${{ number_format($user->earning_wallet, 2) }}</span>
                        </div>
                    ...
                    <div>
                        <label
                            class="block text-[11px] font-extrabold text-amber-400 uppercase tracking-wider mb-1">WITHDRAWAL
                            AMOUNT ($)</label>
                        <div class="relative">
                            <span
                                class="absolute left-3.5 top-1/2 -translate-y-1/2 text-amber-400 font-bold font-mono text-xs">$</span>
                            <input type="number" step="0.01" min="10" name="amount" id="withdrawAmount" required
                                placeholder="Minimum $10.00"
                                class="w-full pl-8 pr-3 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-mono font-bold text-xs focus:outline-none focus:border-amber-400">
                        </div>
                        <span class="text-[10px] text-neutral-400 mt-1 block">Min withdrawal: <strong
                                class="text-amber-400">$10.00</strong></span>
                    </div>
                    ...
                    <div class="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-1.5 text-xs">
                        ...
                        <div
                            class="flex justify-between border-t border-amber-500/20 pt-1.5 text-xs font-black text-emerald-400">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-[11px] font-extrabold text-amber-400 uppercase tracking-wider">USDT
                                (BEP20) WALLET ADDRESS *</label>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="usdt_address" value="{{ old('usdt_address', $user->wallet_address) }}"
                                required placeholder="USDT BEP20 address (0x...)"
                                class="w-full pl-8 pr-3 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-mono text-xs focus:outline-none focus:border-amber-400">
                        </div>
                        @if(!$user->wallet_address)
                            <span class="text-[10px] text-neutral-400 mt-1 block">Tip: Save your payout address in <a
                                    href="{{ route('user.profile') }}" class="text-amber-400 underline font-bold">My Profile</a>
                                to auto-fill automatically!</span>
                        @endif
                    </div>

                    <button type="submit"
                        class="w-full py-3 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-black font-black text-xs uppercase tracking-wider transition shadow-[0_0_15px_rgba(243,202,82,0.4)] flex items-center justify-center gap-1.5">
            ...
            <div class="w-full lg:w-1/2 bg-panel p-5 sm:p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-4">
                <div class="flex items-center gap-3 border-b border-amber-500/20 pb-3">
                    <div
                        class="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 font-bold">
                        ⚖️
                    </div>
                    <div>
                ...
                <div
                    class="p-4 rounded-2xl bg-black/40 border border-amber-500/20 text-[11px] text-neutral-400 leading-relaxed">
                    <strong class="text-amber-300 block mb-1">Notice:</strong>
                    Withdrawal requests are processed securely after admin verification. Once approved, net funds will
                    arrive in your wallet address.
                </div>
            </div>
