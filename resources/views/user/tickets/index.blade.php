    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-neutral-300 mt-1">Submit support requests, track status, and chat directly with
                    NextGen Forex support.</p>
                </div>

            <a href="{{ route('user.tickets.create') }}"
                class="w-full sm:w-auto px-6 py-3.5 rounded-full pdf-gold-ribbon font-black text-xs uppercase tracking-wider flex items-center justify-center gap-2 shadow-xl hover:scale-105 transition text-black">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3.5">
            <a href="{{ route('user.tickets.index') }}"
                class="p-5 rounded-3xl pdf-package-card relative overflow-hidden group block">
            ...
            <a href="{{ route('user.tickets.index', ['status' => 'answered']) }}"
                class="p-5 rounded-3xl pdf-package-card relative overflow-hidden group block">
        ...
        <div class="p-6 rounded-3xl pdf-package-card space-y-6">
            ...
            <form action="{{ route('user.tickets.index') }}" method="GET"
                class="flex flex-col lg:flex-row items-center justify-between gap-4 pb-4 border-b border-amber-500/20">
                ...
                <div class="flex items-center gap-2.5 overflow-x-auto w-full lg:w-auto shrink-0 pb-1 lg:pb-0">
                    <a href="{{ route('user.tickets.index') }}"
                        class="px-5 py-2 rounded-full text-xs font-black transition whitespace-nowrap {{ !request('status') ? 'bg-amber-400 text-black shadow-md' : 'bg-black/60 text-amber-300 border border-amber-500/50 hover:bg-amber-500/20' }}">
                        All ({{ $openCount + $answeredCount + $closedCount }})
                    </a>
                    <a href="{{ route('user.tickets.index', ['status' => 'open']) }}"
                        class="px-5 py-2 rounded-full text-xs font-black transition whitespace-nowrap {{ request('status') === 'open' ? 'bg-amber-400 text-black shadow-md' : 'bg-black/60 text-amber-300 border border-amber-500/50 hover:bg-amber-500/20' }}">
                        Open ({{ $openCount }})
                    </a>
                    <a href="{{ route('user.tickets.index', ['status' => 'answered']) }}"
                        class="px-5 py-2 rounded-full text-xs font-black transition whitespace-nowrap {{ request('status') === 'answered' ? 'bg-amber-400 text-black shadow-md' : 'bg-black/60 text-amber-300 border border-amber-500/50 hover:bg-amber-500/20' }}">
                        Answered ({{ $answeredCount }})
                    </a>
                    <a href="{{ route('user.tickets.index', ['status' => 'closed']) }}"
                        class="px-5 py-2 rounded-full text-xs font-black transition whitespace-nowrap {{ request('status') === 'closed' ? 'bg-amber-400 text-black shadow-md' : 'bg-black/60 text-amber-300 border border-amber-500/50 hover:bg-amber-500/20' }}">
                        Closed ({{ $closedCount }})
                    </a>
                    </div>
                ...
                <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto shrink-0 justify-end">
                    <select name="category" onchange="this.form.submit()"
                        class="px-4 py-2 rounded-full bg-black border border-amber-500/50 text-white text-xs font-bold focus:outline-none cursor-pointer">
                    ...
                    <div class="relative w-full sm:w-64">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search ticket #, subject..."
                            class="w-full pl-9 pr-4 py-2 rounded-full bg-black border border-amber-500/50 text-white font-semibold text-xs focus:outline-none focus:border-amber-400">
                        </div>
                        </div>
                            ...
                            <tr class="hover:bg-amber-500/10 transition group">
                                ...
                                <td class="p-3">
                                    <div>
                                        ...
                                        <span
                                            class="text-[9px] text-amber-400 font-mono uppercase bg-black/60 px-2 py-0.5 rounded border border-amber-500/20">
                                ...
                                <td class="p-3">
                                    @if($ticket->status === 'answered')
                                        <span
                                            class="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 text-[9px] font-black uppercase animate-pulse">
                                            ANSWERED BY SUPPORT
                                        </span>
                                    @elseif($ticket->status === 'open' || $ticket->status === 'user_reply')
                                        <span
                                            class="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/50 text-[9px] font-black uppercase">
                                            OPEN
                                        </span>
                                    @else
                                        <span
