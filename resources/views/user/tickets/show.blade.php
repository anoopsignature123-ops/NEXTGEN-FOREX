                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        @if($ticket->status === 'answered')
                            <span
                                class="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 text-[10px] font-black uppercase animate-pulse">
                                ANSWERED BY SUPPORT
                            </span>
                        @elseif($ticket->status === 'open' || $ticket->status === 'user_reply')
                            <span
                                class="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/50 text-[10px] font-black uppercase">
                                OPEN / WAITING
                            </span>
                        @else
                            <span
                ...
                <div class="flex items-center gap-3 shrink-0">
                    @if($ticket->status !== 'closed')
                        <form action="{{ route('user.tickets.close', $ticket->id) }}" method="POST"
                            onsubmit="return confirm('Mark this ticket as closed/resolved?');">
                            @csrf
                            <button type="submit"
                        ...
                        <div class="p-5 rounded-3xl pdf-package-card space-y-2 border-2 border-amber-400/80 shadow-2xl relative w-full">
                            <div class="flex items-center justify-between border-b border-amber-500/20 pb-2">
                                <span class="font-black text-amber-300 text-xs font-heading flex items-center gap-1.5">
                                    ...
                                    <span
                                        class="px-2 py-0.5 rounded bg-amber-500 text-black text-[9px] font-black uppercase">Official</span>
                                </span>
                                <span
        ...
        @if($ticket->status !== 'closed')
            <div class="p-6 rounded-3xl pdf-package-card space-y-4">
                ...
                <form action="{{ route('user.tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <textarea name="message" rows="4" placeholder="Type your reply message here..." required
