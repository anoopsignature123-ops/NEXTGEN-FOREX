...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30">
        <div>
            ...
            <p class="text-xs text-neutral-400 mt-1">Track your 10% reward bonuses credited upon achieving team business volume milestones ($1k to $50 Lacs)</p>
        </div>
    </div>
    ...
    <div class="p-3.5 rounded-2xl bg-bg/80 border border-amber-500/40 shadow-lg">
        <form action="{{ route('user.reports.rewards') }}" method="GET" class="flex flex-nowrap items-end gap-3 w-full overflow-x-auto text-xs font-sans pb-1">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <a href="{{ route('user.reports.rewards') }}" class="py-2.5 px-4 rounded-xl bg-black/60 border border-white/60 text-white hover:bg-white/10 font-bold text-xs transition flex items-center justify-center shrink-0">
                    Reset
                </a>
            </div>
        </form>
