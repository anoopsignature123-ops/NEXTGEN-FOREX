...
<div class="w-full space-y-6">
    ...
    <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gold-gradient font-heading">MY PURCHASED PACKAGES HISTORY</h1>
            <p class="text-xs text-neutral-300 mt-1">Review your active NextGen Forex growth plans, daily ROI earnings, and maturity dates.</p>
        </div>

        <a href="{{ route('user.packages.index') }}" class="px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-xs font-black uppercase tracking-wider shadow-lg hover:scale-105 transition shrink-0 flex items-center gap-2">
    ...
    <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6">
        <div class="flex items-center justify-between pb-4 border-b border-amber-500/20">
            <h3 class="text-base font-black text-white font-heading">PURCHASED PACKAGES DIRECTORY</h3>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-bg text-amber-400 uppercase text-xs font-bold font-heading tracking-wider border-b border-amber-500/30">
                    <tr>
                        ...
                        <th class="p-4">PURCHASED DATE & TIME</th>
                    ...
                    <tr class="hover:bg-amber-500/10 transition">
                        <td class="p-4 font-bold text-white">
                            <div class="flex items-center gap-2.5">
                                <span class="w-8 h-8 rounded-lg bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center font-bold text-xs">🚀</span>
                        ...
                        <td class="p-4">
                            <div class="text-xs font-semibold text-white">{{ $userPkg->purchased_at ? $userPkg->purchased_at->format('M d, Y') : 'N/A' }}</div>
                            <div class="text-[10px] text-neutral-400 font-mono">{{ $userPkg->purchased_at ? $userPkg->purchased_at->format('h:i A') : '' }}</div>
                        </td>
