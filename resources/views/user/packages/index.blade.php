...
<div class="w-full space-y-6">
    ...
    @endif
        ...
        <div class="p-6 sm:p-8 rounded-3xl bg-black/90 border-2 border-amber-400 shadow-[0_0_35px_rgba(243,202,82,0.3)] space-y-5">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div>
                    <h3 class="text-base font-black text-amber-400 uppercase tracking-wider font-mono flex items-center gap-2">
            ...
            <div class="flex flex-wrap items-center gap-2 pt-1">
                ...
                <button type="button" onclick="setQuickAmount(10)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$10</button>
                <button type="button" onclick="setQuickAmount(50)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$50</button>
                <button type="button" onclick="setQuickAmount(100)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$100</button>
                <button type="button" onclick="setQuickAmount(250)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$250</button>
                <button type="button" onclick="setQuickAmount(500)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$500</button>
                <button type="button" onclick="setQuickAmount(1000)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$1,000</button>
                <button type="button" onclick="setQuickAmount(5000)" class="px-3.5 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 font-mono text-xs font-bold transition">$5,000</button>
            </div>

            <form action="{{ route('user.packages.buy') }}" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end" onsubmit="return confirm('Confirm activating package for this investment amount?')">
                @csrf
                <div class="md:col-span-5 space-y-1.5">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" min="10" id="quickInvestAmount" name="invested_amount" value="100" class="w-full pl-9 pr-4 py-3.5 rounded-2xl bg-black border-2 border-amber-500/60 text-white font-mono font-black text-lg focus:outline-none focus:border-amber-400" required oninput="calculateQuickPackage(this.value)">
                    </div>
                </div>

                <div class="md:col-span-7 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <div id="quickPkgStatus" class="flex-1 p-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/40 text-xs font-mono font-bold text-emerald-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                        <!-- Filled dynamically by JS -->
                    </div>

                    <button type="submit" class="px-8 py-4 rounded-2xl pdf-gold-ribbon hover:brightness-110 text-black font-black text-sm uppercase tracking-wider shadow-xl flex items-center justify-center gap-2 shrink-0 cursor-pointer">
        ...
        <div class="text-center space-y-2 pt-4">
            <div class="inline-flex items-center justify-center px-8 py-2.5 rounded-full pdf-gold-ribbon text-base font-black uppercase tracking-widest shadow-xl">
        ...
        <div class="grid grid-cols-1 grid-3-cards gap-8 pt-2">
            ...
            @endphp
            <div id="pkgCard_{{ $pkg->id }}" 
                 onclick="setQuickAmount({{ $pkg->min_amount }})" 
                ...
                <div class="space-y-4">
                    @if($hasActive)
                        ...
                        <div class="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-400 text-black text-[10px] font-black uppercase tracking-widest border border-white shadow-[0_0_15px_rgba(16,185,129,0.8)] mx-auto">
                    ...
                    <div id="selectedPill_{{ $pkg->id }}" class="hidden inline-flex items-center gap-1.5 px-4 py-1 rounded-full bg-amber-400 text-black text-[10px] font-black uppercase tracking-widest border border-white shadow-[0_0_15px_rgba(245,158,11,0.8)] mx-auto animate-pulse">
                            ...
                            <div class="pt-2 border-t border-amber-500/20 space-y-1.5 font-mono text-xs">
                                <div class="flex justify-between items-center">
                                    <span class="text-emerald-400 font-sans font-bold flex items-center gap-1">
                                        <span class="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span> Total Active Capital:
                                    </span>
                                    <span class="text-emerald-300 font-black text-sm">${{ number_format($activePkg->total_invested, 2) }}</span>
                                </div>
