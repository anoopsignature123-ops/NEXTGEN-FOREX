...
<div class="w-full space-y-6 font-sans">
    ...
    <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gold-gradient font-heading">FINANCIAL TRANSACTION LOGS</h1>
            <p class="text-xs text-neutral-300 mt-1">Detailed history report of all credits, debits, deposits, package purchases, and wallet activities.</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="px-5 py-3 rounded-xl bg-bg border border-amber-500/40 text-amber-300 text-xs font-bold font-mono flex items-center gap-2 shadow-lg shrink-0">
                ...
                <span>Earning Wallet: <strong class="text-emerald-400 text-sm font-black">${{ number_format($user->earning_wallet, 2) }}</strong></span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-amber-500/20 pb-4">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                <a href="{{ route('user.transactions.index') }}" class="whitespace-nowrap inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition {{ !request('wallet_type') && !request('type') ? 'bg-amber-500 text-black font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-amber-400' }}">
                    All Transactions
                </a>
                <a href="{{ route('user.transactions.index', ['wallet_type' => 'deposit_wallet']) }}" class="whitespace-nowrap inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition {{ request('wallet_type') === 'deposit_wallet' ? 'bg-amber-500 text-black font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-amber-400' }}">
                    💳 Deposit Wallet
                </a>
                <a href="{{ route('user.transactions.index', ['wallet_type' => 'earning_wallet']) }}" class="whitespace-nowrap inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition {{ request('wallet_type') === 'earning_wallet' ? 'bg-amber-500 text-black font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-amber-400' }}">
                    💰 Earning Wallet
                </a>
            </div>

            <!-- Search Form (100% MATCHING REFERENCE UI CARD) -->
            <form action="{{ route('user.transactions.index') }}" method="GET" class="flex flex-nowrap items-end gap-3 text-xs font-sans">
                <div class="relative min-w-[200px]">
                    <i data-lucide="search" class="w-3.5 h-3.5 absolute left-3.5 top-1/2 -translate-y-1/2 text-amber-400"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search Txn # or Remark..." class="w-full pl-9 pr-4 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400">
                </div>
                ...
                <tbody class="divide-y divide-amber-500/20 text-neutral-200">
                    ...
                    <tr class="hover:bg-amber-500/10 transition">
                        ...
                        <td class="p-4 text-xs text-neutral-300">
                            <span class="max-w-[280px] inline-block truncate" title="{{ $txn->description }}">{{ $txn->description }}</span>
                        </td>
                        <td class="p-4">
                    ...
                    <tr>
                        <td colspan="7" class="p-8 text-center text-neutral-400 text-sm">
                            No financial transaction logs found.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
