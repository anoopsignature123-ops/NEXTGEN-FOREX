    ...
    <div class="w-full space-y-6 font-sans">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <div
                    class="px-5 py-3 rounded-2xl bg-black/80 border-2 border-amber-400/80 text-amber-300 text-xs font-bold font-mono flex items-center gap-2 shadow-xl shrink-0">
                    <i data-lucide="shield-check" class="w-4 h-4 text-emerald-400"></i>
                    <span>Status: <strong
                            class="{{ $user->status === 'active' ? 'text-emerald-400' : 'text-amber-400' }} text-xs font-black uppercase">{{ $user->status === 'active' ? 'ACTIVE MEMBER' : 'INACTIVE MEMBER' }}</strong></span>
                    </div>
                    </div>
                    </div>
        ...
        <div class="p-6 rounded-3xl pdf-package-card space-y-6">
            <div
                class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-amber-500/20">
                <div class="flex items-center gap-4">
                    <!-- Bright 3D Gold Initial Badge -->
                    <div
                        class="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-300 via-yellow-400 to-amber-600 text-black font-black text-2xl flex items-center justify-center shadow-xl border-2 border-amber-200 shrink-0">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-1">
                            <span
                                class="px-2.5 py-0.5 rounded bg-black/80 border border-amber-500/40 text-amber-300 font-mono font-bold text-[10px]">CODE:
                            ...
                            <span
                                class="px-2.5 py-0.5 rounded bg-black/80 border border-amber-500/40 text-amber-300 font-mono font-bold text-[10px]">EARNING:
                ...
                <a href="{{ route('user.withdrawals.index') }}"
                    class="px-5 py-2.5 rounded-full pdf-gold-ribbon text-xs font-black uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center gap-2 text-black shrink-0">
        ...
        <div class="p-6 rounded-3xl pdf-package-card space-y-3">
            <div class="flex items-center gap-3 border-b border-amber-500/20 pb-3">
                <div
                    class="w-10 h-10 rounded-2xl pdf-gold-badge text-black font-black text-sm flex items-center justify-center shadow-md shrink-0">
                    🔗
                </div>
                <div>
            ...
            <div class="p-4 rounded-2xl bg-black/80 border border-amber-500/30 space-y-2">
                <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider block">SHAREABLE REFERRAL
                    URL</span>
                <div class="flex flex-col sm:flex-row items-center gap-3">
                    <input type="text" id="directRefInput" readonly value="{{ url('/user/register?sponsor=' . $user->referral_code) }}"
                        class="flex-1 w-full px-4 py-2.5 rounded-xl bg-black border border-amber-500/40 text-amber-300 font-mono text-xs truncate focus:outline-none">
                    <button type="button"
                        onclick="navigator.clipboard.writeText(document.getElementById('directRefInput').value); showToast('Copied!', 'Referral link copied to clipboard.', 'success');"
                    ...
                    <form action="{{ route('user.profile.update') }}" method="POST" class="space-y-4">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}" placeholder="Enter your full name..."
                                class="w-full px-4 py-2.5 rounded-xl bg-black/80 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400"
                                required>
                            </div>

                        <div class="space-y-1">
                            <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Email Address <span
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="mobile" value="{{ old('mobile', $user->mobile) }}" placeholder="Enter mobile phone number..."
                                class="w-full px-4 py-2.5 rounded-xl bg-black/80 border border-amber-500/40 text-white font-mono text-xs focus:outline-none focus:border-amber-400"
                                required>
                            </div>
                        ...
                        <div class="space-y-1 p-3.5 rounded-2xl bg-black/90 border-2 border-amber-500/50">
                            ...
                            <input type="text" name="wallet_address" value="{{ old('wallet_address', $user->wallet_address) }}"
                                ...
                                class="w-full px-4 py-2.5 rounded-xl bg-black border border-amber-500/40 text-amber-300 font-mono text-xs focus:outline-none focus:border-amber-400">
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Referral Code <span
                                    class="text-[10px] text-neutral-400 font-normal">(Read-only)</span></label>
                            <input type="text" value="{{ $user->referral_code }}" readonly
                                class="w-full px-4 py-2.5 rounded-xl bg-black/60 border border-neutral-700 text-amber-300 font-mono font-bold text-xs cursor-not-allowed">
                            </div>

                        <div class="pt-2 flex justify-end">
                            <button type="submit"
                                class="px-6 py-2.5 rounded-full pdf-gold-ribbon font-black text-xs uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center gap-2 text-black">
            ...
            <div class="p-6 rounded-3xl pdf-package-card space-y-5 flex flex-col justify-between w-full">
                <div class="space-y-4">
                    ...
                    <form action="{{ route('user.password.update') }}" method="POST" class="space-y-4">
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Current Password
                                *</label>
                            <div class="relative flex items-center">
                                <input type="password" id="current_password" name="current_password" placeholder="Enter your current password..."
                                    class="w-full pl-4 pr-12 py-2.5 rounded-xl bg-black/80 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400"
                                    required>
                                <button type="button" onclick="togglePasswordVisibility('current_password', 'eye_icon_current')"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 z-20 text-amber-400 hover:text-amber-200 focus:outline-none"
                                    title="Toggle Password Visibility">
                                    <svg id="eye_icon_current" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
                                        stroke="#f3ca52" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                                        class="w-5 h-5 text-amber-400 block">
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Confirm New Password
                                *</label>
                            <div class="relative flex items-center">
