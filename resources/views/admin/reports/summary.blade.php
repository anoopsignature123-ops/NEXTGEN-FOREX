    ...
    <div class="space-y-6">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-xs text-neutral-400 mt-1">High-level financial audit dashboard summarizing payouts across all
                    8 NextGen Forex income streams</p>
                </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            ...
            <a href="{{ route('admin.reports.roi') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
            ...
            <a href="{{ route('admin.reports.direct') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">Direct Income
                        (10%)</span>
            ...
            <a href="{{ route('admin.reports.bonus') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">24H Special
                        Bonus</span>
            ...
            <a href="{{ route('admin.reports.level') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-teal-400 uppercase tracking-wider">Level Income (10
                        Levels)</span>
            ...
            <a href="{{ route('admin.reports.matching') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">Matching Income
                        (5%)</span>
            ...
            <a href="{{ route('admin.reports.direct-salary') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">Direct Salary
                        Income</span>
            ...
            <a href="{{ route('admin.reports.team-salary') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">Team Salary
                        Income</span>
            ...
            <a href="{{ route('admin.reports.rewards') }}"
                class="group bg-panel p-4 rounded-2xl border border-amber-500/30 hover:border-amber-400 shadow-lg transition block">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider">Reward Income
                        (10%)</span>
                    ...
                    <tbody class="divide-y divide-amber-500/20 text-neutral-200">
                        @forelse($recentIncomes as $log)
                            <tr class="hover:bg-amber-500/10 transition">
                                ...
                                <td class="p-4">
                                    @if($log->user)
                                        <a href="{{ route('admin.users.show', $log->user->id) }}" class="group block">
                                            <div class="font-bold text-white group-hover:text-amber-400 transition text-xs">
                                                {{ $log->user->name }}</div>
                                            <div class="text-[11px] text-amber-400 font-mono font-bold">
                                                {{ $log->user->referral_code }}</div>
                                        </a>
                                    @else
                                ...
                                <td class="p-4">
                                    @if($log->type === 'direct_commission' && $log->source_member)
                                        <a href="{{ route('admin.users.show', $log->source_member->id) }}" class="group block">
                                            <div class="font-bold text-neutral-200 group-hover:text-amber-400 transition text-xs">
                                                {{ $log->source_member->name }}
                                            </div>
                                            <div class="text-[11px] text-amber-400/80 font-mono font-bold">
                                                {{ $log->source_member->referral_code }}
                                            </div>
                                        </a>
                                    @elseif($log->user && $log->user->sponsor)
                                        <a href="{{ route('admin.users.show', $log->user->sponsor->id) }}" class="group block">
                                            <div class="font-bold text-neutral-200 group-hover:text-amber-400 transition text-xs">
                                                {{ $log->user->sponsor->name }}
                                            </div>
                                            <div class="text-[11px] text-amber-400/80 font-mono">
                                                {{ $log->user->sponsor->referral_code }}
                                            </div>
                                        </a>
                                    @else
                                ...
                                <td class="p-4">
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-black uppercase bg-amber-500/10 text-amber-300 border border-amber-500/30">
                                ...
                                <td class="p-4">
                                    <span
                                        class="px-2.5 py-1 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 uppercase">
                                        Completed
                                    </span>
                                    </td>
                                    </tr>
