    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-4 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3 w-full lg:w-auto">
                <div
                    class="w-full sm:w-auto px-4 sm:px-5 py-3 rounded-xl bg-black/80 border border-emerald-500/60 text-emerald-300 text-xs font-bold font-mono flex items-center justify-center sm:justify-start gap-2 shadow-xl shrink-0">
        ...
        @if(session('success'))
            <div
                class="p-4 rounded-xl bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 text-xs font-bold flex items-center gap-2 shadow-lg">
        ...
        @if($errors->any())
            <div
                class="p-4 rounded-xl bg-rose-500/20 border border-rose-500/50 text-rose-300 text-xs font-bold space-y-1 shadow-lg">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div
                class="bg-panel p-4 sm:p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6 flex flex-col justify-between">
                <div class="space-y-5">
                    <div class="flex items-center gap-3 border-b border-amber-500/20 pb-4">
                        <div
                            class="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 text-black font-black text-base flex items-center justify-center shadow-md shrink-0">
                            ⚙️
                        </div>
                    ...
                    <div
                        class="p-4 rounded-xl bg-gradient-to-r from-emerald-950/60 via-black to-emerald-950/60 border-2 border-emerald-500/50 space-y-2 shadow-lg">
                        <div class="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2">
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                <span class="font-black text-xs uppercase text-emerald-300 tracking-wider truncate">LIVE
                                    PRODUCTION GATEWAY</span>
                            </div>
                            <span
                                class="px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/60 text-emerald-400 text-[10px] font-black tracking-wider uppercase flex items-center gap-1.5 shrink-0">
                                <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                                ACTIVE
                            </span>
                        </div>
                        <p class="text-[11px] text-neutral-300 leading-relaxed font-medium">
                            System executes real-time HTTP checkout sessions directly via <strong
                                class="text-amber-300">iPaymentWallet API</strong> on Binance Smart Chain (BEP20).
                        </p>
                    </div>

                    <form action="{{ route('admin.gateway-settings.update') }}" method="POST" class="space-y-5"
                        id="gatewaySettingForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-xs font-black text-amber-400 uppercase tracking-wider flex items-center gap-2">
                            ...
                            <div class="relative">
                                <input type="text" name="api_key" value="{{ old('api_key', $settings['api_key'] ?? '') }}"
                                    placeholder="Enter Gateway API Key (pk_...)"
                                    class="w-full px-4 py-3.5 rounded-xl bg-black/80 border-2 border-amber-500/40 text-amber-300 font-mono text-xs focus:outline-none focus:border-amber-400 shadow-inner tracking-wider"
                                    required>
                            </div>
                            <p class="text-[10px] text-neutral-400 leading-normal">
                                Required for generating live USDT deposit addresses and checking instant confirmation
                                statuses.
                            </p>
                        </div>
                    </form>
                ...
                <div class="pt-4 border-t border-amber-500/20">
                    <button type="submit" form="gatewaySettingForm"
            ...
            <div
                ...
                <div class="space-y-5">
                    <div class="flex items-center gap-3 border-b border-amber-500/20 pb-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3
                                ...
                                LIVE GATEWAY OPERATIONAL GUIDE</h3>
                    ...
                    <div
                        class="p-4 sm:p-5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 space-y-3 text-xs text-neutral-300 shadow-lg overflow-hidden">
                        ...
                        <div class="space-y-2.5 text-neutral-300 leading-relaxed text-[11px] sm:text-xs">
                            <div class="flex items-start gap-2.5">
                                ...
                                <span>System communicates directly with <strong
                                        class="text-emerald-400 font-mono break-all">https://ipaymentwallet.com/api</strong>.</span>
                            </div>
                            <div class="flex items-start gap-2.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0"></span>
                                <span>Deposit checkouts generate dynamic Binance Smart Chain (BEP20) USDT wallet
                                    addresses.</span>
                            </div>
                            <div class="flex items-start gap-2.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0"></span>
                                <span>Real-time automated polling confirms user transactions on the blockchain before
                                    updating member deposit balances.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="p-4 sm:p-5 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-3 text-xs text-neutral-300 shadow-lg overflow-hidden">
                        ...
                        <div class="space-y-2.5 text-neutral-300 leading-relaxed text-[11px] sm:text-xs">
                            <div class="flex items-start gap-2.5">
                                ...
                                <span>Ensure your <strong class="text-amber-300 font-bold">iPaymentWallet API Key</strong>
                                    is active and valid.</span>
                            </div>
                            <div class="flex items-start gap-2.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-amber-400 mt-1.5 shrink-0"></span>
                                <span>Changes saved in this panel apply immediately across all user deposit
                                    transactions.</span>
                            </div>
                        </div>
                    </div>
