    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-4 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
                ...
                <h1 class="text-xl sm:text-3xl font-black text-gold-gradient font-heading">EDIT MEMBER:
                    {{ $user->referral_code }}
                </h1>
                <p class="text-xs text-neutral-300 mt-1">Update profile information, binary leg placement, or reset password
                </p>
            </div>
            <a href="{{ route('admin.users') }}"
                class="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-panel border border-amber-500/40 text-amber-300 font-bold text-xs uppercase tracking-wider hover:bg-amber-500/20 transition flex items-center justify-center gap-2 shrink-0">
