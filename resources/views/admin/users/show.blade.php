...
<div class="w-full space-y-6 font-sans">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-5 rounded-3xl pdf-package-card relative overflow-hidden">
            <div class="text-[11px] font-extrabold text-neutral-300 uppercase tracking-wider">DEPOSIT WALLET</div>
            <h3 class="text-2xl font-black text-emerald-400 font-mono mt-1">${{ number_format($user->deposit_wallet, 2) }}</h3>
            <span class="text-[10px] text-neutral-400 font-mono block mt-1">Available for investments</span>
        </div>

        <div class="p-5 rounded-3xl pdf-package-card relative overflow-hidden">
            <div class="text-[11px] font-extrabold text-amber-400 uppercase tracking-wider">EARNING WALLET</div>
            <h3 class="text-2xl font-black text-amber-300 font-mono mt-1">${{ number_format($user->earning_wallet, 2) }}</h3>
            <span class="text-[10px] text-neutral-400 font-mono block mt-1">Available for withdrawals</span>
        </div>

        <div class="p-5 rounded-3xl pdf-package-card relative overflow-hidden">
            <div class="text-[11px] font-extrabold text-sky-400 uppercase tracking-wider">ACTIVE INVESTMENT</div>
            <h3 class="text-2xl font-black text-sky-300 font-mono mt-1">${{ number_format($totalInvested, 2) }}</h3>
            <span class="text-[10px] text-neutral-400 font-mono block mt-1">Total active contracts</span>
        </div>

        <div class="p-5 rounded-3xl pdf-package-card relative overflow-hidden">
            <div class="text-[11px] font-extrabold text-teal-400 uppercase tracking-wider">TOTAL EARNINGS ACCUMULATED</div>
            <h3 class="text-2xl font-black text-teal-300 font-mono mt-1">${{ number_format($totalEarnings, 2) }}</h3>
            <span class="text-[10px] text-neutral-400 font-mono block mt-1">All historical payouts</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full items-start">
        ...
        <div class="p-6 rounded-3xl pdf-package-card space-y-4">
            ...
            <div class="space-y-3 text-xs">
                ...
                <div class="flex justify-between items-center py-1.5 border-b border-amber-500/10">
                    <span class="text-neutral-400 font-medium">Placement Branch:</span>
                    <span class="font-black text-amber-300 font-mono uppercase bg-black/60 px-2 py-0.5 rounded border border-amber-500/20">
                        {{ $user->position ? ($user->position === 'left' ? 'Team A' : 'Team B') : 'Team A' }}
                    </span>
                </div>
                <div class="flex justify-between items-center py-1.5 border-b border-amber-500/10">
