    ...
    <div class="w-full space-y-6 font-sans">
        ...
        <div class="ng-banner-title p-4 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-xs text-amber-400 font-extrabold tracking-[3px] uppercase">ADMIN MEMBER PROFILE
                        AUDIT</span>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 text-xs text-neutral-300 mt-1 font-mono">
                    ...
                    <span>Sponsor: <strong
                            class="text-white font-black">{{ $user->sponsor ? $user->sponsor->name . ' (' . $user->sponsor_code . ')' : ($user->sponsor_code ?? 'No Sponsor (N/A)') }}</strong></span>
                    <span class="hidden sm:inline">•</span>
                    <span>Registered: <strong
                            class="text-neutral-300 font-semibold">{{ $user->created_at ? $user->created_at->format('d M Y') : 'N/A' }}</strong></span>
                </div>
            </div>

            <div class="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                <button onclick="openAddFundModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->referral_code }}')"
                    class="flex-1 sm:flex-none px-4 py-2.5 rounded-xl pdf-gold-ribbon font-black text-xs uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center justify-center gap-2 text-black whitespace-nowrap">
                ...
                <a href="{{ route('admin.users.impersonate', $user->id) }}" target="_blank"
                    class="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-bold text-xs uppercase tracking-wider hover:bg-emerald-500/30 transition flex items-center justify-center gap-2 whitespace-nowrap">
                    <i data-lucide="external-link" class="w-4 h-4"></i> Login as User
                </a>
                <a href="{{ route('admin.users.edit', $user->id) }}"
                    class="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-sky-500/20 border border-sky-500/40 text-sky-300 font-bold text-xs uppercase tracking-wider hover:bg-sky-500/30 transition flex items-center justify-center gap-2 whitespace-nowrap">
                    <i data-lucide="edit-3" class="w-4 h-4"></i> Edit Member
                </a>
                <a href="{{ route('admin.users') }}"
                    class="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-black/80 border border-amber-500/40 text-amber-300 font-bold text-xs uppercase tracking-wider hover:bg-amber-500/20 transition flex items-center justify-center gap-2 whitespace-nowrap">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="p-3.5 sm:p-4 rounded-2xl pdf-package-card relative overflow-hidden">
                <div class="text-[11px] font-extrabold text-neutral-300 uppercase tracking-wider truncate">DEPOSIT WALLET
                </div>
            ...
            <div class="p-3.5 sm:p-4 rounded-2xl pdf-package-card relative overflow-hidden">
                <div class="text-[11px] font-extrabold text-amber-400 uppercase tracking-wider truncate">EARNING WALLET
                </div>
            ...
            <div class="p-3.5 sm:p-4 rounded-2xl pdf-package-card relative overflow-hidden">
                <div class="text-[11px] font-extrabold text-sky-400 uppercase tracking-wider truncate">ACTIVE INVESTMENT
                </div>
            ...
            <div class="p-3.5 sm:p-4 rounded-2xl pdf-package-card relative overflow-hidden">
                <div class="text-[11px] font-extrabold text-teal-400 uppercase tracking-wider truncate">TOTAL EARNINGS
                    ACCUMULATED</div>
                <h3 class="text-xl sm:text-2xl font-black text-teal-300 font-mono mt-0.5">
                    ${{ number_format($totalEarnings, 2) }}</h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full items-start">
            ...
            <div class="p-6 rounded-3xl pdf-package-card space-y-4">
                <h3
                    class="text-base font-black text-white flex items-center justify-between border-b border-amber-500/20 pb-3 font-heading uppercase">
                    <span class="flex items-center gap-2"><i data-lucide="user" class="w-5 h-5 text-amber-400"></i> Account
                        & Profile Information</span>
                    @if($user->status === 'active')
                        <span
                            class="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/50 text-[10px] font-black uppercase">ACTIVE</span>
                    @else
                        <span
                            class="px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 border border-rose-500/50 text-[10px] font-black uppercase">INACTIVE</span>
                    @endif
                </h3>
                ...
                <div class="space-y-3 text-xs">
                    ...
                    <div class="flex justify-between items-center py-1.5">
                        ...
                        <span
                            class="font-bold text-neutral-300 font-mono">{{ $user->created_at ? $user->created_at->format('M d, Y h:i A') : 'N/A' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 rounded-3xl pdf-package-card space-y-4">
                <h3
                    class="text-base font-black text-white flex items-center justify-between border-b border-amber-500/20 pb-3 font-heading uppercase">
                    <span class="flex items-center gap-2"><i data-lucide="share-2" class="w-5 h-5 text-emerald-400"></i>
                        Referral & Network Information</span>
                    <span
                        class="px-2.5 py-0.5 rounded bg-black/60 text-amber-300 border border-amber-500/30 text-[10px] font-mono font-bold">
                ...
                <div class="space-y-3.5 text-xs">
                    ...
                    <div class="p-4 rounded-2xl bg-black/80 border border-amber-500/30 space-y-2">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <button onclick="copyLink('{{ url('/user/register?sponsor=' . $user->referral_code) }}', 'Referral Link copied!')"
                                class="px-3 py-1 rounded-xl pdf-gold-ribbon font-black text-[10px] uppercase flex items-center gap-1 shrink-0 shadow text-black">
                                <i data-lucide="copy" class="w-3 h-3 text-black font-black"></i> Copy Link
                            </button>
                        </div>
                        <p class="text-[11px] text-amber-300 font-mono break-all font-bold">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full items-start">
            ...
            <div class="p-6 rounded-3xl pdf-package-card space-y-4 w-full">
                <div class="flex items-center justify-between border-b border-amber-500/20 pb-3">
                    ...
                    <span class="text-xs text-neutral-400 font-mono">Total Contracts: <strong
                            class="text-amber-400 font-black">{{ $user->userPackages->count() }}</strong></span>
                </div>
                            ...
                            @forelse($user->userPackages as $pkg)
                                <tr class="hover:bg-amber-500/10 transition">
                                    <td class="p-3 font-bold text-white flex items-center gap-2">
                                        <span
                                            class="w-2 h-2 rounded-full {{ $pkg->status === 'active' ? 'bg-emerald-400 animate-pulse' : 'bg-neutral-500' }}"></span>
                                        📦 {{ $pkg->package->name ?? 'Investment Package' }}
                                        </td>
                                    <td class="p-3 font-mono font-black text-emerald-400 text-sm">
                                        ${{ number_format($pkg->invested_amount, 2) }}</td>
                                    <td class="p-3 font-mono text-amber-300 font-bold">+{{ $pkg->daily_roi }}% / Day</td>
                                    <td class="p-3 font-mono text-white font-bold">
                                        ${{ number_format($pkg->total_return_amount, 2) }} (2X)</td>
                                    <td class="p-3 font-mono text-neutral-300">
                                        {{ $pkg->purchased_at ? $pkg->purchased_at->format('M d, Y h:i A') : 'N/A' }}
                                    </td>
                    ...
                    <table class="w-full text-left text-xs whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-amber-500/10 text-neutral-300 font-medium">
                            @forelse($user->transactions as $txn)
                                <tr class="hover:bg-amber-500/10 transition">
                                    <td class="p-3 font-mono font-bold text-amber-400">{{ $txn->trx_id ?? 'TXN-' . $txn->id }}
                                    </td>
                                    <td class="p-3">
                                        <span
                                            class="px-2 py-0.5 rounded font-mono text-[9px] font-black uppercase {{ $txn->type === 'credit' ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-300 border border-rose-500/40' }}">
                                            {{ strtoupper($txn->type) }}
                                        </span>
                                        </td>
                                    ...
                                    <td class="p-3 font-mono text-neutral-400 text-[11px]">
    ...
    <div id="addFundModal"
        ...
        <div class="bg-panel border border-amber-500/40 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-5 relative">
            ...
            <div class="flex items-center gap-3 border-b border-amber-500/20 pb-3">
                <div
                    class="w-10 h-10 rounded-full pdf-gold-badge text-black font-black text-sm flex items-center justify-center shadow-md shrink-0">
                    💰
                </div>
                <div>
            ...
            <form id="addFundForm" action="" method="POST" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Transaction Remark /
                        Note</label>
                    <input type="text" name="remark" value="Directly credited by Admin side" placeholder="Enter custom remark..."
                        class="w-full px-4 py-3 rounded-xl bg-black/90 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400"
                        required>
                    <p class="text-[10px] text-neutral-400">This remark will be logged in the member's transaction history.
                    </p>
                </div>

                <div class="pt-2 flex items-center justify-end gap-3">
                    <button type="button" onclick="closeAddFundModal()"
                        class="px-5 py-2.5 rounded-xl bg-black/80 border border-neutral-700 text-neutral-300 font-bold text-xs hover:bg-neutral-800 transition">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-6 py-2.5 rounded-xl pdf-gold-ribbon text-xs font-black uppercase tracking-wider shadow-lg hover:scale-105 transition text-black">
