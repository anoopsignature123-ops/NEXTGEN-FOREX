...
@section('content')
        ...
        <div class="ng-banner-title p-4 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-xs text-amber-400 font-extrabold tracking-[3px] uppercase">NEXTGEN FOREX
                        NETWORK</span>
                </div>
                <h1 class="text-xl sm:text-3xl font-black text-gold-gradient font-heading">USER MANAGEMENT MODULE</h1>
                <p class="text-xs text-neutral-300 mt-1">Inspect registered accounts, active investment packages, wallet
                    balances, and direct fund credits.</p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
                ...
                <!-- Search Form -->
                <form id="userSearchForm" action="{{ route('admin.users') }}" method="GET"
                    ...
                    <div class="relative flex-1 sm:w-64 lg:w-72">
                        <svg xmlns="http://www.w3.org/2000/svg"
                            class="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-amber-400" viewBox="0 0 24 24"
                        ...
                        <input type="text" id="liveSearchInput" name="search" value="{{ request('search') }}"
                            ...
                            class="w-full pl-9 pr-4 py-2.5 rounded-xl bg-bg border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400">
                    </div>
                    <button type="submit"
                        class="px-4 py-2.5 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs hover:bg-amber-500/30 transition flex items-center justify-center gap-1.5 shrink-0">
        ...
        <div class="bg-panel p-4 sm:p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-5">
            ...
            <div class="border-b border-amber-500/20 pb-3.5 overflow-x-auto">
                <div class="flex items-center gap-2 whitespace-nowrap min-w-max">
                    <a href="{{ route('admin.users', array_merge(request()->except('status'), ['status' => ''])) }}"
                        class="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl text-xs font-bold transition {{ !request('status') ? 'bg-amber-500 text-black font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-amber-400' }}">
                    ...
                    <a href="{{ route('admin.users', array_merge(request()->except('status'), ['status' => 'active'])) }}"
                        class="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl text-xs font-bold transition {{ request('status') === 'active' ? 'bg-emerald-500 text-black font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-emerald-400' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
                            <circle cx="9" cy="7" r="4" />
                    ...
                    <a href="{{ route('admin.users', array_merge(request()->except('status'), ['status' => 'inactive'])) }}"
                        class="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-xl text-xs font-bold transition {{ request('status') === 'inactive' ? 'bg-rose-500/20 text-rose-300 border border-rose-500/50 font-black shadow-md' : 'bg-bg border border-amber-500/30 text-neutral-300 hover:text-rose-400' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
                            <circle cx="9" cy="7" r="4" />
                            ...
                            <tr class="hover:bg-amber-500/10 transition user-row">
                                ...
                                <td class="p-4 min-w-[200px]">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-[11px] text-amber-400 font-mono">{{ $user->mobile ?? 'No Mobile' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="p-4 min-w-[180px]">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-1.5">
                                            <span
                                                class="font-black text-amber-400 font-mono text-sm user-code">{{ $user->referral_code }}</span>
                                            <button
                                                onclick="copyToClipboard('{{ $user->referral_code }}', 'Referral Code copied!')"
                                                class="p-1 rounded hover:bg-amber-500/20 text-amber-400 transition"
                                        ...
                                        <div class="flex items-center gap-1">
                                            ...
                                            <button
                                                onclick="copyToClipboard('{{ url('/user/register?sponsor=' . $user->referral_code) }}', 'Referral Link copied!')"
                                                class="text-[11px] text-amber-300 hover:underline font-mono flex items-center gap-1"
                                        ...
                                        <div class="space-y-1">
                                            ...
                                            <div class="text-xs font-mono font-bold text-emerald-400">
                                                ${{ number_format($activePkg->invested_amount, 2) }} <span
                                                    class="text-[10px] text-neutral-400 font-normal">({{ $activePkg->daily_roi }}%
                                                    Daily ROI)</span>
                                            </div>
                                        </div>
                                    @else
                                ...
                                <td class="p-4 min-w-[160px] font-mono text-xs space-y-1">
                                    <div class="flex items-center gap-1.5 text-neutral-300">
                                        ...
                                        <strong
                                            class="text-emerald-400 font-black">${{ number_format($user->deposit_wallet, 2) }}</strong>
                                    </div>
                                    <div class="flex items-center gap-1.5 text-neutral-300">
                                        <span class="text-[10px] font-sans text-neutral-400">Earning:</span>
                                        <strong
                                            class="text-amber-300 font-black">${{ number_format($user->earning_wallet, 2) }}</strong>
                                    </div>
                                </td>
                                ...
                                <td class="p-4 min-w-[120px]">
                                    ...
                                    @else
                                        <span
                                            class="px-2.5 py-1 rounded bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[10px] font-black uppercase">INACTIVE</span>
                                    @endif
                                </td>
                                ...
                                <td class="p-4 min-w-[140px]">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/20 border border-amber-500/50 text-amber-300 text-[10px] font-extrabold uppercase">
                                ...
                                <td class="p-4 min-w-[290px]">
                                    <div class="flex items-center justify-start gap-1.5 whitespace-nowrap">
                                        ...
                                        <button
                                            onclick="openAddFundModal('{{ $user->id }}', '{{ $user->name }}', '{{ $user->referral_code }}')"
                                            ...
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-black"
                                        ...
                                        <a href="{{ route('admin.network.direct', ['search' => $user->referral_code]) }}"
                                            class="p-2 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 transition shadow flex items-center justify-center shrink-0"
                                            title="View Direct Members Sponsored By User">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-amber-400"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round">
                                                <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
    ...
    <div id="addFundModal"
        class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm hidden flex items-center justify-center p-3 sm:p-4">
        <div
            class="bg-panel border border-amber-500/40 rounded-2xl p-5 sm:p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl space-y-5 relative animate-in fade-in zoom-in duration-200">
            ...
            <form id="addFundForm" action="" method="POST" class="space-y-4">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-bold text-amber-400 uppercase tracking-wider">Transaction Remark /
                        Note</label>
                    <input type="text" name="remark" value="Directly credited by Admin side"
                        placeholder="Enter custom remark..."
                        class="w-full px-4 py-3 rounded-xl bg-bg border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400"
                        required>
                ...
                <div class="pt-2 flex items-center justify-end gap-3">
                    <button type="button" onclick="closeAddFundModal()"
                        class="px-5 py-3 rounded-xl bg-bg border border-neutral-700 text-neutral-300 font-bold text-xs hover:bg-neutral-800 transition">
                        Cancel
                    </button>
                    <button type="submit"
                        class="px-6 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg transition flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-black" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="10" />
                            <line x1="12" y1="8" x2="12" y2="16" />
