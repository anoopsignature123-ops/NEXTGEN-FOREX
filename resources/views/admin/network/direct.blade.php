    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.network.tree') }}"
                    class="px-5 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg hover:scale-105 transition flex items-center gap-2">
        ...
        <div class="pdf-package-card p-6 shadow-2xl rounded-2xl space-y-6">
            <!-- Search & Filter Form -->
            <form action="{{ route('admin.network.direct') }}" method="GET"
                class="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-amber-500/20 pb-4">
                <div class="relative w-full sm:w-72">
                    <i data-lucide="search" class="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-amber-400"></i>
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Search name, code, sponsor..."
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    <select name="position" onchange="this.form.submit()"
                        class="px-4 py-2.5 rounded-xl bg-bg border border-amber-500/40 text-amber-400 font-bold text-xs focus:outline-none cursor-pointer">
                    ...
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="inactive" {{ request('status') === 'inactive' ? 'selected' : '' }}>Inactive Members
                        </option>
                    </select>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead
                        class="bg-black/80 text-amber-400 uppercase text-xs font-bold font-heading tracking-wider border-b border-amber-500/30">
                        <tr>
                        ...
                        @forelse($directs as $direct)
                            <tr class="hover:bg-amber-500/10 transition">
                                <td class="p-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full pdf-gold-badge text-black font-black text-xs flex items-center justify-center shadow">
                                ...
                                <td class="p-4">
                                    @if(strtolower((string) $direct->position) === 'left')
                                        <span
                                            class="px-2.5 py-1 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[10px] font-black uppercase">TEAM
                                            A</span>
                                    @elseif(strtolower((string) $direct->position) === 'right')
                                        <span
                                            class="px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-[10px] font-black uppercase">TEAM
                                            B</span>
                                    @else
                                        <span
                                            class="px-2.5 py-1 rounded bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[10px] font-black uppercase">DIRECT</span>
                                    @endif
                                </td>
                                <td class="p-4 text-xs font-medium text-neutral-300">
                                    {{ $direct->created_at ? $direct->created_at->format('M d, Y h:i A') : 'N/A' }}</td>
                                <td class="p-4">
                                ...
                                <td class="p-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.users.impersonate', $direct->id) }}" target="_blank"
                                            class="px-3.5 py-1.5 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 hover:border-amber-400 transition flex items-center gap-1.5 text-xs font-bold shadow"
                                            title="Login as User in New Tab">
                                            <i data-lucide="external-link" class="w-4 h-4 text-amber-400"></i> Login as User
                                        </a>
                                        <a href="{{ route('admin.network.tree', ['code' => $direct->referral_code]) }}"
                                            class="p-2 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 hover:border-amber-400 transition shadow flex items-center justify-center"
