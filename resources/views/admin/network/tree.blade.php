        ...
        <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6">
            <div class="flex items-center justify-between border-b border-amber-500/20 pb-4">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-black text-amber-400 font-mono">{{ $rootUser->name }}
                        ({{ $rootUser->referral_code }})</span>
                    </div>

                <a href="{{ route('admin.network.tree') }}"
                    class="px-3 py-1.5 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/40 hover:bg-amber-500/40 transition text-xs font-bold flex items-center gap-1">
