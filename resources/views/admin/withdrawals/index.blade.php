                                    ...
                                    <td class="p-4 text-center">
                                        @if($w->status === 'pending')
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <button type="submit" formaction="{{ route('admin.withdrawals.approve', $w->id) }}"
                                                    onclick="return confirm('Approve withdrawal request {{ $w->trx_number }}? Status will change from Pending to Approved.');"
                                                    class="px-3 py-1.5 rounded-lg bg-sky-500/20 hover:bg-sky-500 text-sky-300 hover:text-white border border-sky-500/40 font-bold text-xs transition flex items-center gap-1 shadow">
                                                    <i data-lucide="check" class="w-3.5 h-3.5"></i> Approve
                                                </button>

                                                <!-- Reject Button -->
                                                <button type="submit" formaction="{{ route('admin.withdrawals.reject', $w->id) }}"
                                                    onclick="return confirm('Reject withdrawal and refund ${{ number_format($w->amount, 2) }} back to user earning wallet?');"
                                                    class="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/40 font-bold text-xs transition flex items-center gap-1 shadow">
                                        ...
                                        @elseif($w->status === 'approved')
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <button type="button"
                                                    onclick="promptComplete('{{ route('admin.withdrawals.complete', $w->id) }}', '{{ number_format($w->net_amount, 2) }}')"
                                                    class="px-3 py-1.5 rounded-lg bg-emerald-500/20 hover:bg-emerald-500 text-emerald-300 hover:text-black border border-emerald-500/40 font-bold text-xs transition flex items-center gap-1 shadow">
                                                    <i data-lucide="check-check" class="w-3.5 h-3.5"></i> Mark Complete
                                                </button>

                                                <!-- Reject Button -->
                                                <button type="submit" formaction="{{ route('admin.withdrawals.reject', $w->id) }}"
                                                    onclick="return confirm('Reject approved withdrawal and refund ${{ number_format($w->amount, 2) }} back to user earning wallet?');"
                                                    class="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/40 font-bold text-xs transition flex items-center gap-1 shadow">
                                                    <i data-lucide="x" class="w-3.5 h-3.5"></i> Reject
                                                </button>
                                            </div>
