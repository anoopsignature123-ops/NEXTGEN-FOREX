    ...
    <div class="w-full space-y-6 font-sans">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-xs text-amber-400 font-extrabold tracking-[3px] uppercase">FINANCIAL REPORT &
                        LOGS</span>
                </div>
                <h1 class="text-2xl sm:text-3xl font-black text-gold-gradient font-heading">PACKAGE INVESTMENT HISTORY</h1>
                <p class="text-xs text-neutral-300 mt-1">Audit member investment contracts, daily ROI yields, 2X cap
                    progress, and status logs across the network.</p>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('admin.packages.index') }}"
                    class="px-4 py-2.5 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs hover:bg-amber-500/30 transition flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-panel p-5 rounded-2xl border border-amber-500/30 shadow-xl space-y-2">
                <div class="flex items-center justify-between">
                    ...
                    <div
                        class="w-8 h-8 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center">
            ...
            <div class="bg-panel p-5 rounded-2xl border border-amber-500/30 shadow-xl space-y-2">
                <div class="flex items-center justify-between">
                    <span class="text-[11px] font-black text-neutral-400 uppercase tracking-wider">Total Capital
                        Invested</span>
                    <div
                        class="w-8 h-8 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center">
                        <i data-lucide="dollar-sign" class="w-4 h-4 text-emerald-400"></i>
                    </div>
                </div>
                <div class="text-2xl font-black text-emerald-400 font-mono">${{ number_format($totalCapitalInvested, 2) }}
                </div>
        ...
        <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6">
            ...
            <div class="p-3.5 rounded-2xl bg-bg/80 border border-amber-500/40 shadow-lg">
                <form action="{{ route('admin.packages.history') }}" method="GET"
                    class="flex flex-nowrap items-end gap-3 w-full overflow-x-auto text-xs font-sans pb-1">
                    ...
                    <div class="w-44 shrink-0">
                        <label
                            class="block text-[10px] font-extrabold text-amber-400 uppercase tracking-wider mb-1">PACKAGE</label>
                        <select name="package_id"
                            class="w-full px-3 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400 cursor-pointer">
                    ...
                    <div class="w-36 shrink-0">
                        <label
                            class="block text-[10px] font-extrabold text-amber-400 uppercase tracking-wider mb-1">STATUS</label>
                        <select name="status"
                            class="w-full px-3 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400 cursor-pointer">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <a href="{{ route('admin.packages.history') }}"
                            ...
                            <tr class="hover:bg-amber-500/10 transition">
                                ...
                                <td class="p-4 min-w-[200px]">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 text-black font-black text-xs flex items-center justify-center shadow-md shrink-0">
                                            {{ strtoupper(substr($inv->user->name ?? 'U', 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="font-black text-white text-sm">{{ $inv->user->name ?? 'Deleted User' }}
                                            </div>
                                            <div class="text-xs text-neutral-400">{{ $inv->user->email ?? 'N/A' }}</div>
                                            <div class="text-[11px] text-amber-400 font-mono">
                                                {{ $inv->user->referral_code ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="p-4 min-w-[140px] font-mono">
                                    <div class="text-sm font-black text-emerald-400">
                                        ${{ number_format($inv->invested_amount, 2) }}</div>
                                ...
                                <td class="p-4 min-w-[150px] font-mono">
                                    ...
                                    <div class="text-[11px] text-emerald-400 font-bold">
                                        ${{ number_format($inv->daily_roi_amount, 2) }} / day</div>
                                </td>
                                ...
                                <td class="p-4 min-w-[170px] font-mono">
                                    <div class="text-xs font-bold text-amber-400">
                                        Paid: <strong
                                            class="text-emerald-400 font-black">${{ number_format($inv->paid_roi_amount, 2) }}</strong>
                                    </div>
                                ...
                                <td class="p-4 min-w-[150px]">
                                    <div class="text-xs font-semibold text-white">
                                        {{ $inv->purchased_at ? $inv->purchased_at->format('M d, Y') : 'N/A' }}</div>
                                    <div class="text-[11px] text-neutral-400 font-mono">
                                        {{ $inv->purchased_at ? $inv->purchased_at->format('h:i A') : '' }}</div>
                                </td>
                                ...
                                <td class="p-4 min-w-[120px]">
                                    @if($inv->status === 'active')
                                        <span
                                            class="px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 w-fit">
                                            <span class="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span>
                                            ACTIVE
                                        </span>
                                    @elseif($inv->status === 'completed')
                                        <span
                                            class="px-2.5 py-1 rounded bg-sky-500/20 text-sky-300 border border-sky-500/40 text-[10px] font-black uppercase tracking-wider w-fit block">
                                            COMPLETED (2X MET)
                                        </span>
                                    @else
                                        <span
                                            class="px-2.5 py-1 rounded bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[10px] font-black uppercase tracking-wider w-fit block">
                                ...
                                <td class="p-4 min-w-[120px] text-center">
                                    @if($inv->user)
                                        <a href="{{ route('admin.users.show', $inv->user->id) }}"
                                            class="px-3 py-1.5 rounded-xl border border-amber-500/60 bg-amber-500/10 text-amber-300 hover:bg-amber-500/30 transition text-xs font-bold flex items-center justify-center gap-1"
