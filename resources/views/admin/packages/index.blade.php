    ...
    <div class="w-full space-y-6">
        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-xs text-neutral-300 mt-1">Configure investment tiers, daily ROI percentages, contract
                    duration, and status.</p>
                </div>
        ...
        <div class="bg-panel p-6 shadow-2xl rounded-2xl border border-amber-500/30 space-y-6">
            ...
            <div class="grid grid-cols-1 grid-3-cards gap-6">
                @foreach($packages as $pkg)
                    <div
                        class="p-6 rounded-3xl bg-bg border border-amber-500/40 space-y-4 relative overflow-hidden group flex flex-col justify-between hover:border-amber-400/80 transition shadow-xl">
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <form action="{{ route('admin.packages.toggle-status', $pkg->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="px-3 py-1 rounded-full text-[10px] font-black uppercase border transition {{ $pkg->status === 'active' ? 'bg-amber-500/20 text-amber-300 border-amber-500/40 hover:bg-amber-500/30' : 'bg-rose-500/20 text-rose-300 border-rose-500/40 hover:bg-rose-500/30' }}">
                            ...
                            <div>
                                <div class="text-xs font-extrabold text-neutral-300 uppercase tracking-wider">{{ $pkg->name }}
                                </div>
                                <h3 class="text-2xl sm:text-3xl font-black text-gold-gradient font-mono mt-1">
                                    ${{ number_format($pkg->min_amount, 0) }} - ${{ number_format($pkg->max_amount, 0) }}</h3>
                                </div>
                            ...
                            <div class="p-4 rounded-2xl bg-panel border border-amber-500/30 space-y-2 font-mono">
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span class="text-amber-300 font-black text-sm">{{ number_format($pkg->total_return_multiplier, 1) }}X
                                        Return</span>
                                    </div>
                                    </div>
                        ...
                        <div class="pt-3 border-t border-amber-500/20">
                            <a href="{{ route('admin.packages.edit', $pkg->id) }}"
                                class="w-full py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black text-xs uppercase tracking-wider shadow-lg hover:scale-[1.02] transition text-center flex items-center justify-center gap-1.5">
