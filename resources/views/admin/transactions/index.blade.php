        ...
        <div class="ng-banner-title p-6 sm:p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-xs text-amber-400 font-extrabold tracking-[3px] uppercase">NEXTGEN FOREX AUDIT &
                        REPORTS</span>
                </div>
                <h1 class="text-2xl sm:text-3xl font-black text-gold-gradient font-heading">SYSTEM TRANSACTION HISTORY</h1>
                <p class="text-xs text-neutral-300 mt-1">Audit, track, and inspect all financial transactions, deposits,
                    package purchases, and wallet credits.</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <div
                    ...
                    <span>Total Credits: <strong
                            class="text-emerald-400 text-sm font-black">${{ number_format($totalCredits, 2) }}</strong></span>
                </div>
                <div
                    class="px-5 py-3 rounded-xl bg-bg border border-amber-500/40 text-amber-300 text-xs font-bold font-mono flex items-center gap-2 shadow-lg shrink-0">
                    <i data-lucide="arrow-up-right" class="w-4 h-4 text-rose-400"></i>
                    <span>Total Debits: <strong
                            class="text-rose-400 text-sm font-black">${{ number_format($totalDebits, 2) }}</strong></span>
                </div>
            </div>
        </div>
        ...
        <div class="p-3.5 rounded-2xl bg-bg/80 border border-amber-500/40 shadow-lg">
            <form action="{{ route('admin.transactions.index') }}" method="GET"
                ...
                <div class="w-44 shrink-0">
                    <label class="block text-[10px] font-extrabold text-amber-400 uppercase tracking-wider mb-1">SELECT
                        WALLET</label>
                    <select name="wallet_type"
                        ...
                        <option value="deposit_wallet" {{ request('wallet_type') === 'deposit_wallet' ? 'selected' : '' }}>💳
                            Deposit Wallet</option>
                        <option value="earning_wallet" {{ request('wallet_type') === 'earning_wallet' ? 'selected' : '' }}>💰
                ...
                <!-- 4. TRANSACTION TYPE -->
                <div class="w-44 shrink-0">
                    <label class="block text-[10px] font-extrabold text-amber-400 uppercase tracking-wider mb-1">TRANSACTION
                        TYPE</label>
                    <select name="type"
                        class="w-full px-3 py-2.5 rounded-xl bg-black/60 border border-amber-500/40 text-white font-semibold text-xs focus:outline-none focus:border-amber-400 cursor-pointer">
                        <option value="">All Types</option>
                        <option value="deposit" {{ request('type') === 'deposit' ? 'selected' : '' }}>Deposit Approval
                        </option>
                        <option value="package_purchase" {{ request('type') === 'package_purchase' ? 'selected' : '' }}>
                            Package Purchase</option>
                        <option value="admin_add_fund" {{ request('type') === 'admin_add_fund' ? 'selected' : '' }}>Admin
                            Direct Credit</option>
                        <option value="daily_roi" {{ request('type') === 'daily_roi' ? 'selected' : '' }}>Daily ROI Credit
                        </option>
                        <option value="direct_commission" {{ request('type') === 'direct_commission' ? 'selected' : '' }}>
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Name, code, TXN-..."
                            ...
                            <tr class="hover:bg-amber-500/10 transition">
                                ...
                                <td class="p-4">
                                    @if($txn->user)
                                        <div class="flex items-center gap-3">
                                            <div
                                                class="w-8 h-8 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 font-bold text-xs flex items-center justify-center shrink-0">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-[11px] text-amber-400 font-mono flex items-center gap-1">
                                                    ...
                                                    <a href="{{ route('admin.users.show', $txn->user->id) }}" class="text-neutral-400 hover:text-amber-300"
                                ...
                                <td class="p-4">
                                    <div class="text-xs font-semibold text-white">
                                        {{ $txn->created_at ? $txn->created_at->format('M d, Y') : 'N/A' }}
                                    </div>
