...
<div class="relative group-node inline-block text-center">
    ...
    <div class="desktop-node-tooltip hidden md:block absolute bottom-[108%] left-1/2 -translate-x-1/2 mb-2 w-72 sm:w-80 p-4 rounded-3xl border-2 border-[#f3ca52] shadow-[0_20px_50px_rgba(0,0,0,0.95)] text-left space-y-2 text-xs"
        ...
        <div class="p-2 rounded-xl bg-black/60 border border-sky-500/40 flex justify-between items-center text-[11px]">
            ...
            <span class="font-black text-sky-300 font-mono">{{ $rightStats['active'] }} Act / {{ $rightStats['inactive'] }} Inact</span>
        </div>
        ...
        <a href="{{ route($routePrefix . '.network.tree', ['code' => $node->referral_code]) }}" 
           class="block p-3.5 rounded-3xl pdf-package-card text-center {{ $isSmall ? 'w-32 p-2.5' : 'w-44' }} transition-all duration-300 hover:scale-105">
            ...
            <div class="relative {{ $isSmall ? 'w-9 h-9' : 'w-11 h-11' }} mx-auto mb-2">
                <div class="w-full h-full rounded-full pdf-gold-badge text-black font-black {{ $isSmall ? 'text-xs' : 'text-base' }} flex items-center justify-center shadow-lg">
